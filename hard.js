document.addEventListener('DOMContentLoaded', function() {
    const grid = document.querySelector('.gameGridHard')
    const flagRemaining = document.querySelector('#flagsRemaining')
    const result = document.querySelector('#result')
    const statsDisplay = document.querySelector('#stats3')
    ...
    function applyBoardColor(color) {
        ...
        grid.className = `gameGridHard ${color}`;
    }
    ...
    function createGameBoard() {  
        ...
        for(let i = 0; i <squares.length; i++) {
            ...
            if (squares[i].classList.contains('safe')) {
                if (i > 0 && !isLeftCorner && squares[i - 1].classList.contains('bomb')) total++
                if (i > width - 1 && !isRightCorner && squares[i + 1 - width].classList.contains('bomb')) total++
                if (i > width - 1 && squares[i - width].classList.contains('bomb')) total++
                if (i > width && !isLeftCorner && squares[i - width - 1].classList.contains('bomb')) total++
                if (i < width * width - 1 && !isRightCorner && squares[i + 1].classList.contains('bomb')) total++
                if (i < width * (width - 1) && !isLeftCorner && squares[i - 1 + width].classList.contains('bomb')) total++
                if (i < width * (width - 1) && !isRightCorner && squares[i + 1 + width].classList.contains('bomb')) total++
    ...
    function checkedSquare(square) {
        ...
        setTimeout(() => {
            if (currentId > 0 && !isLeftCorner) click(squares[currentId - 1])
            if (currentId > width - 1 && !isRightCorner) click(squares[currentId + 1 - width])
            if (currentId > width - 1) click(squares[currentId - width])
            if (currentId > width && !isLeftCorner) click(squares[currentId - 1 - width])
            if (currentId < width * width - 1 && !isRightCorner) click(squares[currentId + 1])
            if (currentId < width * (width - 1) && !isLeftCorner) click(squares[currentId - 1 + width])
            if (currentId < width * (width - 1) && !isRightCorner) click(squares[currentId + 1 + width])
